...
pub enum SGPError {
	...
	make_nearest_sampler_failed = C.SGP_ERROR_MAKE_NEAREST_SAMPLER_FAILED
...
// VsAttrLocation is C.sgp_vs_attr_location
...
@[typedef]
pub struct C.sgp_textured_rect {
pub mut:
	dst Rect
	src Rect
}
...
pub type ColorUb4 = C.sgp_color_ub4

@[typedef]
pub struct C.sgp_vertex {
pub mut:
	position Vec2
	texcoord Vec2
	color    ColorUb4
...
@[typedef]
pub struct C.sgp_textures_uniform {
pub mut:
	count u32
	// TODO 	images [SGP_TEXTURE_SLOTS]gfx.Image
	// TODO 	samplers [SGP_TEXTURE_SLOTS]C.sg_sampler
}

pub type TexturesUniform = C.sgp_textures_uniform

@[typedef]
pub struct C.sgp_state {
pub mut:
	frame_size    Isize
	viewport      Irect
	scissor       Irect
...
// C: `SOKOL_GP_API_DECL void sgp_set_image(int channel, sg_image image);/* Sets current bound image in a texture channel. */`
fn C.sgp_set_image(channel int, image gfx.Image)

// set_image sets current bound image in a texture channel.
pub fn set_image(channel int, image gfx.Image) {
	C.sgp_set_image(channel, image)
}

// C: `SOKOL_GP_API_DECL void sgp_unset_image(int channel);/* Remove current bound image in a texture channel (no texture). */`
fn C.sgp_unset_image(channel int)

// unset_image removes current bound image in a texture channel (no texture).
pub fn unset_image(channel int) {
	C.sgp_unset_image(channel)
}

// C: `SOKOL_GP_API_DECL void sgp_reset_image(int channel);/* Resets current bound image in a texture channel to default (white texture). */`
fn C.sgp_reset_image(channel int)

// reset_image resets current bound image in a texture channel to default (white texture).
pub fn reset_image(channel int) {
	C.sgp_reset_image(channel)
}

// C: `SOKOL_GP_API_DECL void sgp_set_sampler(int channel, sg_sampler sampler);/* Sets current bound sampler in a texture channel. */`
fn C.sgp_set_sampler(channel int, sampler C.sg_sampler)

// set_sampler sets current bound sampler in a texture channel.
pub fn set_sampler(channel int, sampler C.sg_sampler) {
	C.sgp_set_sampler(channel, sampler)
}

// C: `SOKOL_GP_API_DECL void sgp_reset_sampler(int channel);/* Resets current bound sampler in a texture channel to default (nearest sampler). */`
fn C.sgp_reset_sampler(channel int)

// reset_sampler resets current bound sampler in a texture channel to default (nearest sampler).
...
// C: `SOKOL_GP_API_DECL void sgp_draw(sg_primitive_type primitive_type, const sgp_vertex* vertices, uint32_t count);/* Low level drawing function, capable of drawing any primitive. */`
fn C.sgp_draw(primitive_type gfx.PrimitiveType, const_vertices &Vertex, count u32)

// draw lows level drawing function, capable of drawing any primitive.
pub fn draw(primitive_type gfx.PrimitiveType, const_vertices &Vertex, count u32) {
	C.sgp_draw(primitive_type, const_vertices, count)
}
...
// C: `SOKOL_GP_API_DECL void sgp_draw_textured_rects(int channel, const sgp_textured_rect* rects, uint32_t count);/* Draws a batch textured rectangle, each from a source region. */`
fn C.sgp_draw_textured_rects(channel int, const_rects &TexturedRect, count u32)

// draw_textured_rects draws a batch textured rectangle, each from a source region.
pub fn draw_textured_rects(channel int, const_rects &TexturedRect, count u32) {
	C.sgp_draw_textured_rects(channel, const_rects, count)
}

// C: `SOKOL_GP_API_DECL void sgp_draw_textured_rect(int channel, sgp_rect dest_rect, sgp_rect src_rect);/* Draws a single textured rectangle from a source region. */`
fn C.sgp_draw_textured_rect(channel int, dest_rect Rect, src_rect Rect)

// draw_textured_rect draws a single textured rectangle from a source region.
